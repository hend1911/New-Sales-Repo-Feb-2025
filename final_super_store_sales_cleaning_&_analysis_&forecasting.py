...
edges = [
    ("Sales (Fact Table)", "Customers (Dim)"),
    ("Sales (Fact Table)", "Products (Dim)"),
    ("Sales (Fact Table)", "Locations (Dim)"),
]
...
# Get colors from a palette (e.g., 'viridis')
palette = sns.color_palette("viridis", n_colors=len(tables) + 1)  # +1 for Ship Mode
node_colors = [palette[i] for i in range(len(tables))]

# Draw nodes and edges with improved styling
nx.draw_networkx_nodes(G, pos, node_color=node_colors, node_size=4000, edgecolors="black", linewidths=1.5)
nx.draw_networkx_edges(G, pos, edge_color="gray", arrows=True, arrowstyle="-|>", arrowsize=15, width=1.5)
nx.draw_networkx_labels(G, pos, font_size=10, font_weight="bold", font_color="black", verticalalignment="center")

# Add label for Ship Mode inside Sales node
sales_pos = pos["Sales (Fact Table)"]
plt.text(sales_pos[0], sales_pos[1] - 0.05, "Ship Mode", ha="center", va="center", fontsize=9, fontweight="bold", color=palette[-1])  # Use last color from palette

# Add a title and adjust layout
plt.title("Database Schema (Star Schema Model)", fontsize=16, fontweight="bold", pad=20)
plt.axis("off")
plt.tight_layout()
...
!pip install --force-reinstall "pmdarima==2.0.3" "numpy==1.25.0"
import pandas as pd
...
# Load your data (assuming 'orders_table' is your DataFrame)
orders_table = pd.read_csv('orders_table.csv')  # Replace with your file path
orders_table['order_date'] = pd.to_datetime(orders_table['order_date'])
...
plt.plot(pd.date_range(start=profit_monthly.index[-1], periods=12, freq='M'), forecast_profit, label='Forecast', marker='o', linestyle='--', color='red', linewidth=2)
...
except AttributeError:  # Some ARIMA models might not have get_forecast method
    pass
...
# Load the orders_table.csv file:
orders_table = pd.read_csv('orders_table.csv') # Load the orders_table data
...
# Generate forecast
forecast_profit = model_profit.predict
...
# Assuming 'orders_table' contains your data
# Load the products_table.csv file:  # This line is added to load products_table
products_table = pd.read_csv('products_table.csv')

# Merge orders_table with products_table to get the 'Category' column
# Assuming 'Product ID' is the common key
orders_table = pd.merge(orders_table, products_table[['Product ID', 'Category']], on='Product ID', how='left')

# Create the sales_monthly DataFrame by grouping and aggregating sales by year and month
# Include 'Category' in the aggregation to keep it in the DataFrame
sales_monthly = orders_table.groupby(['Year', 'Month', 'Category'])['Sales'].sum().reset_index()

# Now you can create the 'Year-Month' column in sales_monthly
...
# ----> Get the category columns from X_train <----
...
# Assuming 'orders_table' contains your original data
# ... (Code to calculate 'customer_counts_yearly' from 'orders_table' should be added here) ...

# Example code to calculate 'customer_counts_yearly' (You might need to modify this):
customer_counts_yearly = orders_table.groupby('Year')['Customer ID'].nunique().reset_index()
# Rename the 'Customer ID' column to 'Customer_Count'
customer_counts_yearly.rename(columns={'Customer ID': 'Customer_Count'}, inplace=True)

# Add example columns for 'New Customers %' and 'Returning Customers %' (replace with your actual calculations)
# The error was here: The list had 3 values but the DataFrame has 4 rows
# Solution: Add one more value to the list to match the number of rows
customer_counts_yearly['New Customers %'] = [10, 15, 12, 18]  # Example values, added one more value
customer_counts_yearly['Returning Customers %'] = [80, 85, 88, 82]  # Example values, added one more value
...
# Convert 'order_date' to datetime if it's not already
historical_data['order_date'] = pd.to_datetime(historical_data['order_date'])  # Convert to datetime
...
encoder = LabelEncoder()  # Assuming encoder is a LabelEncoder instance

# Use the 'values' attribute to get the underlying NumPy array of the Series
merged_df["Ship Mode Encoded"] = encoder.fit_transform(merged_df[target.replace("Ship Mode", "Ship_Mode")].values.reshape(-1, 1))
